...
var (
	...
	FGServiceClass = `C:\Users\Michael\go\src\leprechaun1\material_design\android\src\main\java\com\lormann\leprechaun\ForegroundService.java`
	// ANDROID ...
)
		...
		case e := <-win.window.Events():
			...
			case system.FrameEvent:
				...
				for closeButton.Clicked() {
					botBtnClicked++
					if win.botStage == Stopped {
						win.botStage = Running
					} else {
						win.botStage = Stopped
					}
					if botBtnClicked < 2 {
...
func (win *Window) handleStartStop(userEvent bool) {
	...
	if win.botStage == Running && !botIsStopping {
		logView.SetText("")
